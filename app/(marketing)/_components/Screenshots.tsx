...
function ScreenshotPlaceholder({ label }: { label: string }) {
  return (
    <div className="w-full bg-gradient-to-br from-gray-900 to-gray-800 flex flex-col items-center justify-center gap-2 py-16">
      <span className="text-xs font-bold tracking-widest uppercase" style={{ color: 'rgba(255,255,255,0.3)' }}>Screenshot</span>
      <span className="text-base font-bold italic" style={{ color: 'rgba(255,255,255,0.12)' }}>{label}</span>
    </div>
  )
}
...
const screenshots = [
  {
    ...
    cards: [
      ...
      {
        placeholder: 'Image 4 — Mentor Program',
  ...
  },
  {
    ...
    cards: [
      {
        placeholder: 'Image 2 — Team Settings / White-Label Theming',
      ...
      {
        placeholder: 'Image 9 — Alumni Profile',
            ...
            <div key={row} className="grid md:grid-cols-2 gap-6">
              {cards.map(card => (
                <div key={card.title} className="rounded-sm bg-brand-black border border-white/5 overflow-hidden">
                  <ScreenshotPlaceholder label={card.placeholder} />
