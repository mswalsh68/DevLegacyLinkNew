...
export function Select({
  label, value, onChange, options, required, disabled, error,
}: SelectProps) {
  const id      = useId()
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
      ...
      >
        {options.map((opt) => (
          <option
            key={opt.value !== '' ? opt.value : `__placeholder__${opt.label}`}
            value={opt.value}
          >
            {opt.label}
          </option>
      ...
      {error && (
        <span id={errorId} role="alert" style={{ fontSize: 12, color: 'var(--color-danger)' }}>
          {error}
        </span>
      )}
    </div>
