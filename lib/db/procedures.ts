...
export async function sp_UpdatePlayer(params: {
  userId:                string
  updatedBy:             string
  // Name/identity fields — not yet in SP; accepted here so routes compile
  firstName?:            string
  lastName?:             string
  recruitingClass?:      number
  homeTown?:             string
...
/**
 * Maps role name strings (used throughout the app layer) to the integer role_id
 * values defined in dbo.roles (migration 018). Used by sp_CreateUser / sp_UpdateUser
 * wrappers so callers can pass a role name rather than a raw int.
 */
const ROLE_NAME_TO_ID: Record<string, number> = {
  platform_owner:  1,
  global_admin:    2,   // legacy alias → app_admin
  app_admin:       2,
  head_coach:      3,
  coach_staff:     3,   // legacy alias → head_coach
  position_coach:  4,
  alumni_director: 5,
  player:          6,
  alumni:          7,
  readonly:        7,   // legacy alias → alumni
  read_only:       7,
}
...
export async function sp_GetUsers(params: {
  search?:     string
  globalRole?: string   // role name string — mapped to roleId internally
  page:        number
  pageSize:    number
}): Promise<{ users: UserRow[]; totalCount: number }> {
  const roleId = params.globalRole ? (ROLE_NAME_TO_ID[params.globalRole] ?? null) : null
  const { recordset, output } = await execFull('global', 'sp_GetUsers', (r) => {
    r.input ('Search',     sql.NVarChar(255), params.search ?? null)
    r.input ('RoleId',     sql.Int,           roleId)
    r.input ('Page',       sql.Int,           params.page)
    r.input ('PageSize',   sql.Int,           params.pageSize)
    r.output('TotalCount', sql.Int)
  })
  return {
    users:      (recordset as unknown as UserRow[]) ?? [],
    totalCount: (output.TotalCount as number) ?? 0,
  }
}

export async function sp_CreateUser(params: {
  email:        string
  passwordHash: string
  firstName:    string
  lastName:     string
  ...
  createdBy:    string
  teamId?:      string | null
  grantAppName?: string | null
  grantAppRole?: string | null
}): Promise<{ newUserId: string | null; errorCode: string | null }> {
  const roleId = ROLE_NAME_TO_ID[params.globalRole] ?? 7
  const { output } = await execFull('global', 'sp_CreateUser', (r) => {
    r.input ('Email',        sql.NVarChar(255),  params.email)
    r.input ('PasswordHash', sql.NVarChar(255),  params.passwordHash)
    r.input ('FirstName',    sql.NVarChar(100),  params.firstName)
    r.input ('LastName',     sql.NVarChar(100),  params.lastName)
    r.input ('RoleId',       sql.Int,            roleId)
    r.input ('CreatedBy',    sql.UniqueIdentifier, params.createdBy)
    r.input ('TeamId',       sql.UniqueIdentifier, params.teamId ?? null)
    r.input ('GrantAppName', sql.NVarChar(50),   params.grantAppName ?? null)
    r.input ('GrantAppRole', sql.NVarChar(50),   params.grantAppRole ?? null)
    r.output('NewUserId',    sql.UniqueIdentifier)
    r.output('ErrorCode',    sql.NVarChar(50))
  })
  return {
    newUserId: (output.NewUserId as string | null) ?? null,
    errorCode: (output.ErrorCode as string | null) ?? null,
  }
}

export async function sp_UpdateUser(params: {
  targetUserId: string
  globalRole:   string | null  // role name string — mapped to roleId; null = no change
  isActive:     boolean | null
  actorId:      string
}): Promise<{ errorCode: string | null }> {
  const roleId = params.globalRole ? (ROLE_NAME_TO_ID[params.globalRole] ?? null) : null
  const { output } = await execFull('global', 'sp_UpdateUser', (r) => {
    r.input ('TargetUserId', sql.UniqueIdentifier, params.targetUserId)
    r.input ('RoleId',       sql.Int,              roleId)
    r.input ('IsActive',     sql.Bit,              params.isActive)
    r.input ('ActorId',      sql.UniqueIdentifier, params.actorId)
    r.output('ErrorCode',    sql.NVarChar(50))
  })
  return { errorCode: (output.ErrorCode as string | null) ?? null }
}
...
export async function sp_CreateInviteToken(params: {
  ...
  await exec('global', 'sp_CreateInviteToken', (r) => {
    r.input('UserId',    sql.UniqueIdentifier, params.userId ?? null)
...
export async function sp_GetUserPermissions(params: {
  userId: string
}): Promise<PermissionRow[]> {
  const rows = await exec('global', 'sp_GetUserPermissions', (r) => {
    r.input('UserId', sql.UniqueIdentifier, params.userId)
  })
  return (rows as unknown as PermissionRow[]) ?? []
}

export async function sp_GrantPermission(params: {
  userId:    string
  appName:   string
  role:      string
  grantedBy: string
}): Promise<{ errorCode: string | null }> {
  const { output } = await execFull('global', 'sp_GrantPermission', (r) => {
    r.input ('UserId',    sql.UniqueIdentifier, params.userId)
    r.input ('AppName',   sql.NVarChar(50),     params.appName)
    r.input ('Role',      sql.NVarChar(50),     params.role)
    r.input ('GrantedBy', sql.UniqueIdentifier, params.grantedBy)
    r.output('ErrorCode', sql.NVarChar(50))
  })
  return { errorCode: (output.ErrorCode as string | null) ?? null }
}

// ─── App DB — Campaign detail ─────────────────────────────────────────────────
...
export async function sp_GetCampaignDetail(params: {
  campaignId:         string
  requestingUserId:   string
  requestingUserRole: string
}): Promise<{ campaign: CampaignDetailRow | null; errorCode: string | null }> {
  const { recordset, output } = await execFull('app', 'sp_GetCampaignDetail', (r) => {
  ...
  const rows = recordset as unknown as CampaignDetailRow[]
  return {
